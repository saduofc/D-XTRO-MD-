...
const { getLatestAdaderanaNews } = require('../lib/adaderanaScraper');
...
async function getLatestNews() {
    ...
    try {
        const adaderanaNews = await getLatestAdaderanaNews();
        if (adaderanaNews) {
            newsData.push(adaderanaNews);
        }
    } catch (err) {
        console.error(`Error fetching Adaderana News: ${err.message}`);
    }
